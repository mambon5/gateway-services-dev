...
# save periodic message to db or not
save_periodic_message = 0
...
class RxMsg:
    ...
    def _save_periodic_message_to_db(self, node_id, timestamp):
        """ Save Evaluation app <periodic message> in a sql db. """
        # connecting to database if not already connected
        # if 'conn' not in locals():

        path = "/home/roma/smartec/backend/local/"
        datab = path+'test-time-series.db'
        table = "node_status"
        ...
        # now if table doesn't exist, create it:
            ...
            print("creating SQL table "+table+"...")
        ...
        row = [(node_id, timestamp)]
        conn.executemany("INSERT INTO node_status (node_id, time) \
            VALUES (?, ?)", row);
        conn.commit()
    ...
    def _get_node_id(self):
        """
        Function to parse <node ID> field from Evaluation app message payload and
        initialize class instance node_id attribute.
        """
        node_id = None
    ...
    def display(self, gateway_id, sink_id, node_id, timestamp):
        ...
        print("{} received from gateway <{}>, sink <{}>, node <{}> :".format(
            timestamp, gateway_id, sink_id, node_id))
    ...
    def store(self, gateway_id, sink_id, node_id, timestamp):
        """
        Function to save to database Evaluation app message payload fields.
        ...
        # Print header information of received message.
        print("Storing record to database; node id: {}, date-time: {} ...".format(node_id,timestamp ))
        
        self._save_periodic_message_to_db(node_id, timestamp)
...
class BackendShell(Cmd):
    ...
    def do_store_messages_to_db(self, arg):
        "Write messages to sql database to/from endpoints 1/1. Format: write_messages_to_db"

        # setting the global to true
        global save_periodic_message
        save_periodic_message = 1
        # Register for incoming data and store reception filter ID
        filter_id = self.wni.register_data_cb(on_message_event_data_received_callback,
                                              src_ep=UPLINK_PACKET_EVAL_APP_ENDPOINT,
                                              dst_ep=UPLINK_PACKET_EVAL_APP_ENDPOINT
        ...
        # Wait for user input to stop messages reception.
        input("Press any key to end message writing to database.\n")
    ...
    def do_bye_bye(self, arg):
        "Close network interface and the console and exit. Format: bye bye locos." 
...
def on_message_event_data_received_callback(data):
    ...
    if msg.parse():
        ...
        if save_periodic_message == 1:
            msg.store(data.gw_id, data.sink_id, data.source_address, (data.rx_time_ms_epoch/1000))
